...
pub fn to_be_greater_suite() {
  Suite("When matching to_be_greater", [
    ...
    Test("it should fail when values are equal", fn() {
      ...
      |> expect.to_be_equal(
        garanti.Fail("Expected 1 to be greater than 1", [garanti.Actual("1")]),
      )
    }),
    ...
    Test("it should fail when actual value is smaller", fn() {
      ...
      |> expect.to_be_equal(
        garanti.Fail("Expected 1 to be greater than 3", [garanti.Actual("1")]),
      )
    }),
  ])
}
...
pub fn to_be_greater_or_equal_suite() {
  Suite("When matching to_be_greater_or_equal", [
    ...
    Test("it should fail when actual value is smaller", fn() {
      ...
      |> expect.to_be_equal(
        garanti.Fail("Expected 1 to be greater than or equal to 3", [
          garanti.Actual("1"),
        ]),
      )
    }),
